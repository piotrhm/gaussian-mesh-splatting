...
class FlameConfig:
    def __init__(self):
        ...
        self.flame_lmk_embedding_path='games/flame_splatting/FLAME/model/landmark_embedding_with_eyes.npy',
        self.flame_template_mesh_path='games/flame_splatting/FLAME/model/head_template_mesh.obj',
        self.shape_params = 300
        self.expression_params = 100
        self.expr_params = 100
        self.pose_params = 6
        ...
        self.device = device
        self.f_shape = torch.zeros(1, 300).float().to(device)
        self.f_exp = torch.zeros(1, 100).float().to(device)
        self.f_eyes_pose = torch.zeros(1, 6).float().to(device)
        self.f_neck_pose = torch.zeros(1, 3).float().to(device)
        self.f_jaw_pose = torch.zeros(1, 3).float().to(device)
        self.f_trans = torch.zeros(1, 3).float().to(device)
        self.f_rotation = torch.zeros(1, 3).float().to(device)        
