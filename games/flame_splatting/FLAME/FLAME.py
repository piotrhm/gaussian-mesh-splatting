...
FLAME_PARTS_PATH = "games/flame_splatting/FLAME/model/FLAME_masks.pkl" # FLAME Vertex Masks
...
class FLAME(nn.Module):
    ...
    def __init__(
        ...
        self.n_shape_params = config.shape_params
        self.n_expr_params = config.expr_params
        ...
        shapedirs = torch.cat(
            [shapedirs[:, :, :config.shape_params], shapedirs[:, :, 300 : 300 + config.expr_params]],
            2,
        )
        ...
        lmk_embeddings = np.load(
            config.flame_lmk_embedding_path[0], allow_pickle=True, encoding="latin1"
        )
        ...
        # add faces and uvs
        verts, faces, aux = load_obj(config.flame_template_mesh_path[0], load_textures=False)
